...
<html  lang="${language}">
    <head>
        ...
        <title><c:if test="${isUpdate == true}"><fmt:message key="edit.popup.edit"/><fmt:message key="work.work"/> - Info Support</c:if>
            <c:if test="${isUpdate == false}"><fmt:message key="edit.popup.create"/><fmt:message key="work.work"/> - Info Support</c:if></title>
        <style type="text/css">
            textarea {
            }
            textarea {
    ...
    <body onload="validateForm()">
        <div class="header">
            <h1>
                <c:if test="${isUpdate == true}"><fmt:message key="edit.popup.edit"/> <fmt:message key="work.work"/></c:if>
                <c:if test="${isUpdate == false}"><fmt:message key="edit.popup.create"/> <fmt:message key="work.work"/></c:if>
            </h1>
        </div>
        <c:choose>
            <c:when test="${empty workId}">
                <!-- work id = empty, new work -->
            ...
            function saveForm() {
                if (validateForm()) {
                    ...
                    if (!document.getElementById('workId').value) {
                        document.getElementById('newWork').submit();
                    }
                    else {
                        document.getElementById('editWork').submit();
                    }
                }
            }
        </script>
